...
if __name__ == '__main__':
    ...
    import_table(
        'comments',
        'comments',
        ['id', 'user_id', 'text', 'date_added', 'obj_id'],
    ...
    for f in spectra_files:
        ...
        try:
            spectrum = Spectrum.from_ascii(
                f, obj_id, instruments[0].id, datetime.strptime(obs_date, '%Y%m%d')
            )
    ...
    phot_info = (
        DBSession()
        .query(sa.sql.functions.min(Photometry.id), Photometry.obj_id)
    ...
    for f in cutout_files:
        ...
        DBSession().add(
            Thumbnail(file_uri=f, type=thumb_type, photometry_id=phot_map[obj_id])
        )
        DBSession().commit()
