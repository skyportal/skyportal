...
from skyportal.models import (init_db, Base, DBSession, ACL, Comment,
                              ...
                              Token, Event, GcnNotice, Tag, Localization,
                              Plan, Tele, Field, SubField)
...
class GCNHandler():
    ...
    def download(self, url, dateobs):
        ...
        localization = Localization.query.filter_by(
            dateobs=dateobs, localization_name=filename).all()
    ...
    def contour(self, localization_name, dateobs):
        localization = Localization.query.filter_by(
            dateobs=dateobs, localization_name=localization_name).all()[0]
        ...
        levels = [50, 90]
        paths = ligo.skymap.postprocess.contour(cls, levels,
    ...
    def get_dateobs(self):
        ...
        dateobs = Time(self.root.find("./WhereWhen/{*}ObsDataLocation"
                                      "/{*}ObservationLocation"
                                      "/{*}AstroCoords"
    ...
    def get_tags(self):
        ...
        classifications = [
            ...
            self.root.iterfind("./What/Group[@type='Classification']/Param")]
        if classifications:
...
def create_fields():
    ...
    with tqdm(telescopes) as telescope_progress:
        for tele in telescope_progress:
            ...
            DBSession().merge(Tele(telescope=tele,
            ...
            for field_id, ra, dec in tqdm(fields, 'populating fields'):
                ...
                DBSession().merge(Field(telescope=tele,
                                        field_id=int(field_id),
            ...
            if tele == "ZTF":
                ...
                for field_id, xyz in zip(
                    ...
                    for ii, xyz in enumerate(xyz):
                        ...
                        DBSession().merge(SubField(telescope=tele,
                                                   field_id=int(field_id),
                                                   subfield_id=int(ii),
                                                   ipix=ipix.tolist()))
