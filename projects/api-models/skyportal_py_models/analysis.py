...
class ObjAnalysisResponse(BaseModel):
    ...
    # ``model_lightcurve``, ``model_lightcurves``, ``model_name``,
    # ``n_detections`` and the three ``model_spectrum*`` keys are injected by
    # the handler rather than being columns.
    ...
    # Shaped by the analysis service, so kept loose like model_lightcurve.
    model_spectrum: Any = None
    model_spectrum_summary: str | None = None
    model_spectrum_source: Any = None
