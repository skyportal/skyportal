...
# and want to set as the status of the submission + notify the user
...
def build_reporters_and_remarks_string(submission_request, source, tnsrobot, session):
    ...
    """
    from skyportal.models import TNSRobotCoauthor, User
...
def process_submission_request(submission_request, session):
    ...
    try:
        ...
        if tnsrobot.source_group_id not in TNS_SOURCE_GROUP_NAMING_CONVENTIONS:
            raise TNSReportError(
                f"Unknown naming convention for TNS source group ID {tnsrobot.source_group_id}, cannot validate object ID."
            )
        regex_pattern = TNS_SOURCE_GROUP_NAMING_CONVENTIONS[tnsrobot.source_group_id]
        if not re.match(regex_pattern, obj_id):
            raise TNSReportError(
                f"Object ID {obj_id} does not match the expected naming convention for TNS source group ID {tnsrobot.source_group_id}."
            )
...
def validate_submission_requests():
    ...
    session_context_id.set(uuid.uuid4().hex)
    from skyportal.models import DBSession, Obj, TNSRobot, TNSRobotSubmission
