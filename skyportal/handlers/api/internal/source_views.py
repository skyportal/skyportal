...
from ....models import DBSession, Source, SourceView, GroupSource
...
class SourceViewsHandler(BaseHandler):
    ...
    def get(self):
        ...
        q = (
            DBSession.query(
                func.count(SourceView.source_id).label("views"), SourceView.source_id
            )
            .group_by(SourceView.source_id)
            .filter(
                SourceView.source_id.in_(
