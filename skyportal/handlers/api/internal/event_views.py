...
from ....models import DBSession, Obj, Event, EventView


default_prefs = {'maxNumEvents': 10, 'sinceDaysAgo': 1000}
...
class EventViewsHandler(BaseHandler):
    ...
    def get(self):
        ...
        q = (
            DBSession.query(
                func.count(EventView.event_id).label('views'), EventView.event_id
            )
...
def register_event_view(event_id, username_or_token_id, is_token):
    sv = EventView(
        event_id=event_id, username_or_token_id=username_or_token_id, is_token=is_token
    )
    DBSession.add(sv)
    DBSession.commit()
