...
def params_struct(
    ...
    if doCompletedObservations:
        ...
        observations = models.Observation.query.filter(
            ...
            & (models.Observation.obstime <= completed_end_time.datetime)
        ).all()
        ...
        if len(coverage_struct["data"]) > 0:
            rows, idx, cnts = np.unique(
                coverage_struct["data"], return_index=True, return_counts=True, axis=0
            )
            if tele == "ZTF":
                idx = idx[cnts >= 32]
            coverage_struct["data"] = coverage_struct["data"][idx, :]
            coverage_struct["filters"] = [coverage_struct["filters"][ii] for ii in idx]
            coverage_struct["ipix"] = [coverage_struct["ipix"][ii] for ii in idx]
    ...
    if doPlannedObservations:
        ...
        r = requests.get(urllib.parse.urljoin(ZTF_URL, 'current_queue'))
        data = r.json()
        ...
        if len(queue) > 0:
            for index, row in queue.iterrows():
                ...
                start_time = time.Time(row["slot_start_time"], format="datetime")
                ...
                if np.isnan(filter_id):
                    filter_id = 2
                f = Field.query.filter_by(telescope=tele, field_id=field_id).one()
                coverage_struct["data"] = np.append(
                    coverage_struct["data"],
                    np.array(
                        [
...
def tile(
    ...
    **plan_args
    ...
    plan = Plan.query.filter_by(
        dateobs=dateobs, plan_name=plan_name, telescope=telescope
    ).first()
    print(plan)
