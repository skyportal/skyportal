...
def send_observation_plan(plan_id, session, auto_send=False, default_obsplan_id=None):
    ...
    if auto_send and default_obsplan_id is not None:
        ...
        if observation_plan_request.created_at < datetime.now(UTC).replace(
            tzinfo=None
        ) - timedelta(hours=1):
            log(
        ...
        if plan_request_end_date:
            if (
                ...
                < datetime.now(UTC).replace(tzinfo=None).timestamp()
            ):
                log(
    ...
    def post(self):
        ...
        with self.Session() as session:
            ...
            if "queue_name" in payload:
                ...
                payload["queue_name"] = (
                    f"ToO_{str(datetime.now(UTC).replace(tzinfo=None)).replace(' ', 'T')}"
                )
