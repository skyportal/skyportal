    ...
    def get(self, tag_identifier=None):
        with self.Session() as session:
            if tag_identifier is None:
                ...
                if tag is None:
                    return self.error(f"Tag with ID {tag_identifier} not found", status=404)
                ...
                tag = session.scalars(
                    ObjTagOption.select(session.user_or_token)
                    .where(ObjTagOption.tag_name == tag_identifier)
                ).first()
                
                if tag is None:
                    return self.error(f"Tag with name '{tag_identifier}' not found", status=404)
...
class ObjTagHandler(BaseHandler):
    ...
    def post(self):
        ...
        with self.Session() as session:
            ...
            if not session.scalars(
                ObjTagOption.select(session.user_or_token)
                .where(ObjTagOption.id == objtagoption_id)
            ).first():
    ...
    def put(self, association_id):
        ...
        if not new_tag_id and not new_source_id:
            return self.error("Either `objtagoption_id` or `source_id` has not been provided for update")

        with self.Session() as session:
            assoc = session.scalars(
                ObjTags.select(session.user_or_token)
                .where(ObjTags.id == association_id)
            ).first()
            ...
            if new_tag_id:
                ...
                if not session.scalars(
                    ObjTagOption.select(session.user_or_token)
                    .where(ObjTagOption.id == new_tag_id)
                ).first():
                    return self.error("Specified tag does not exist", status=404)
