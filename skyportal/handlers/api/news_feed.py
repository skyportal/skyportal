...
class NewsFeedHandler(BaseHandler):
    ...
    def get(self):
        ...
        def fetch_newest(model, include_bot_comments=False, session=DBSession()):
        ...
        with DBSession() as session:
            ...
            if (
                ...
                sources = fetch_newest(Source, session=session)
                source_seen = set()
            ...
            if (
                ...
                comments = fetch_newest(Comment, include_bot_comments, session=session)
                # Add latest comments
            ...
            if (
                ...
                spectra = fetch_newest(Spectrum, session=session)
                # Add latest spectra
                news_feed_items.extend(
                    [
                        {
