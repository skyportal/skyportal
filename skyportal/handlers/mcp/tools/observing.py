    ...
    for tel in tel_list:
        ...
        if not np.any(observable):
            ...
            if peak_alt > 0:  # type: ignore
                transit_utc = time_grid[peak_idx].datetime.replace(tzinfo=UTC)  # type: ignore
        ...
        for ri, si in zip(rise_indices, set_indices):
            rise_utc = time_grid[ri].datetime.replace(tzinfo=UTC)  # type: ignore
            set_utc = time_grid[si].datetime.replace(tzinfo=UTC)  # type: ignore
