...
def test_clause_scopes_to_the_event_when_given():
    params = []
    clause = _delta_t_clause(2, None, "2025-10-31 21:07:49", True, None, params)
...
def test_clause_restricts_to_readable_groups():
    params = []
    clause = _delta_t_clause(2, None, None, False, [3, 4], params)
...
def test_clause_denies_when_no_group_is_readable():
    ...
    params = []
    clause = _delta_t_clause(2, None, None, False, [], params)
    assert "AND false" in clause


def test_max_delta_t_absent_by_default():
    assert SourceGetQuery().maxDeltaT is None


def test_max_delta_t_accepts_a_window():
    assert SourceGetQuery(maxDeltaT=2).maxDeltaT == 2


def test_max_delta_t_refuses_a_negative_window():
    with pytest.raises(ValidationError):
        SourceGetQuery(maxDeltaT=-1)


def test_prefix_keeps_the_two_cuts_apart():
    # Both cuts can be set at once, so their bind parameters must not collide.
    params = []
...
def test_max_delta_t_clause_bounds_the_age():
    params = []
    clause = _delta_t_clause(5, None, None, True, None, params, prefix="maxdt")
    assert "abs((value ->> 'delta_t')::float) <= :maxdt_delta_t" in clause
