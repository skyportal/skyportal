...
def test_news_feed(
    ...
    for i in range(2):
        ...
        status, data = api(
            "POST",
            "comment",
            data={"source_id": f"{source_id_base}_{i}", "text": f"comment_text_{i}"},
            token=comment_token,
        )
        assert status == 200
    ...
    for i in range(2):
        ...
        driver.wait_for_xpath(
            f'//span[contains(text(),"comment_text_{i} ({source_id_base}_{i})")]'
        )
...
def test_news_feed_prefs_widget(
    ...
    driver.wait_for_xpath_to_disappear(
        f'//span[text()="New source {source_id_base}_0"]'
    )
    prefs_widget_button.click()
    n_items_input = driver.wait_for_xpath('//input[@name="numItems"]')
    n_items_input.clear()
