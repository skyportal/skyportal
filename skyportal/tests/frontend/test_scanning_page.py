...
def test_candidate_group_filtering(
    driver, user, public_candidate, public_group, upload_data_token, manage_groups_token
):
...
def test_candidate_date_filtering(
    driver, user, public_candidate, public_group, upload_data_token
):
    candidate_id = str(uuid.uuid4())
    for i in range(20):
        status, data = api(
...
def test_save_candidate_single_group_membership(
    driver, group_admin_user, public_group, upload_data_token, manage_groups_token
):
    candidate_id = str(uuid.uuid4())
    status, data = api(
        "POST",
    ...
    status, data = api(
        "POST",
        "groups",
        data={"name": str(uuid.uuid4()), "group_admins": [group_admin_user.username]},
        token=manage_groups_token,
    )
    assert status == 200
    ...
    first_save_button.click()
    assert (
        driver.wait_for_xpath(f"//input[@name='{public_group.id}']").get_attribute(
            "checked"
        )
        == "true"
    )
    assert (
        driver.wait_for_xpath(f"//input[@name='{other_group_id}']").get_attribute(
            "checked"
        )
        is None
    )
    ...
    status, data = api("GET", f"sources/{candidate_id}", token=upload_data_token)
    assert [g["id"] for g in data["data"]["sources"]["groups"]] == [public_group.id]


def test_save_candidate_multiple_group_membership(
    driver, group_admin_user, public_group, upload_data_token, manage_groups_token
):
    candidate_id = str(uuid.uuid4())
    status, data = api(
    ...
    status, data = api("GET", f"sources/{candidate_id}", token=upload_data_token)
    assert [g["id"] for g in data["data"]["sources"]["groups"]] == [
        public_group.id,
        other_group_id,
    ]
