...
def test_add_user_to_group(
    driver, user, super_admin_user, public_group, public_group2, capsys
):
    with capsys.disabled():
        driver.get(f'/become_user/{super_admin_user.id}')
        driver.get('/user_management')
        # It can take a long time to load all the users for user management in a test run
        # as users tend to be created anew for each test.
        driver.wait_for_xpath(
            f"//div[@id='deleteGroupUserButton_{user.id}_{public_group.id}']"
        )
