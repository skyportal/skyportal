...
def test_delete_stream_user(driver, super_admin_user, user, stream_with_users):
    ...
    # The Streams column lives in the data grid alongside the Roles/ACLs/Groups
    # columns; click_xpath waits for the chip's delete icon to be clickable and
    # resolves it fresh right before clicking, matching the other chip-delete
    # tests above. (An earlier scroll_to_element_and_click(scroll_parent=True)
    # variant called scrollIntoView() inside the virtualized grid scroller,
    # which recycled the row and left the icon element stale, so the click was
    # silently swallowed and the notification never appeared.)
    driver.click_xpath(
