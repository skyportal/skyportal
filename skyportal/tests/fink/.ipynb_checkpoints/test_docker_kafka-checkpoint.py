...
def test_kafka_producer():
    ...
    test = subprocess.Popen(['docker-compose', 'up', '-d'], cwd=basedir, preexec_fn=os.setsid)
    output = test.communicate()[0]
    assert test.returncode == 0
...
def test_skyportal_api(super_admin_token):
    ...
    assert status == 200
    status, id = sa.post_instruments('ztf','imager',1, ['ztfr', 'ztfg', 'ztfi'], super_admin_token)
    assert status == 200
    ...
    if instrument in instruments.keys():
        ...
        if ztf_id not in candidate_ids:
            ...
            print(id_filter)
            status, ids = sa.post_candidate(ztf_id, ra, dec, [id_filter], passed_at, token=super_admin_token)
            assert status == 200
        instrument_id = instruments[instrument]
        time.sleep(2)
        status, id = sa.post_photometry(ztf_id, mjd, instrument_id, filter, mag, magerr, limiting_mag, magsys, ra, dec, [id_fink], [id_stream], token=super_admin_token)
        assert status == 200
        
        with open(taxonomy_dir) as file:
            tax = yaml.load(file, Loader=yaml.FullLoader)
        status, taxonomy_id = sa.post_taxonomy(name='demo_taxonomy', hierarchy= tax[0]['hierarchy'], version='1.0.0', token=super_admin_token)
        assert status == 200
        
        if sa.classification_exists_for_objs(ztf_id, token=super_admin_token):
            status = sa.update_classification(ztf_id, topic, probability, taxonomy_id, [groups_dict[topic]], token=super_admin_token)
        else:
            print('this classification doesnt exist yet')
            status, classification_id = sa.post_classification(ztf_id, topic, probability, taxonomy_id, [groups_dict[topic]], token=super_admin_token)
    else:
