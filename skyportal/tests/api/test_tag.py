...
def test_modify_tag(super_admin_token):
    ...
    status, data = api(
        "PUT", f"objtagoption/{tag_id}", data=data, token=super_admin_token
    )
    assert status == 200
    ...
    status, data = api(
        "PUT", f"objtagoption/{tag_id}", data="", token=super_admin_token
    )
    assert status == 500
    ...
    status, data = api(
        "PUT", f"objtagoption/9999999", data=data, token=super_admin_token
    )
    assert status == 404
...
def test_delete_tag(super_admin_token):
    ...
    verify_status, verify_tag = api(
        "GET", f"objtagoption/{tag_id}", token=super_admin_token
    )
...
def test_update_association(super_admin_token):
    ...
    assoc_data = {"objtagoption_id": tag1["data"]["id"], "source_id": 1}
    ...
    update_data = {"objtagoption_id": tag2["data"]["id"]}
    status, data = api(
        "PUT", f"objtag/999999999", data=update_data, token=super_admin_token
    )
    assert status == 404
...
def test_delete_association(super_admin_token):
    ...
    assoc_data = {"objtagoption_id": tag["data"]["id"], "source_id": 1}
    _, assoc = api("POST", "objtag", data=assoc_data, token=super_admin_token)
