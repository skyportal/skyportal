...
def test_token_user_retrieving_source_photometry(view_only_token, public_source):
    status, data = api(
        "GET", f"sources/{public_source.id}/photometry", token=view_only_token
    )
    assert status == 200
    assert data["status"] == "success"
...
def test_token_user_post_new_source(upload_data_token, view_only_token, public_group):
    ...
    status, data = api("GET", f"sources/{source_id}", token=view_only_token)
    assert status == 200
...
def test_delete_source_cascade_photometry(manage_sources_token, public_source):
    ...
    for photometry_id in photometry_ids:
        status, data = api(
            "GET", f"photometry/{photometry_id}", token=manage_sources_token
        )
        assert status == 200

    status, data = api(
        "DELETE", f"sources/{public_source.id}", token=manage_sources_token
    )
    assert status == 200
...
def test_delete_source_cascade_spectra(manage_sources_token, public_source):
    ...
    for spec_id in spec_ids:
        status, data = api("GET", f"spectrum/{spec_id}", token=manage_sources_token)
        assert status == 200
...
def test_delete_source_cascade_comments(manage_sources_token, public_source):
    ...
    for comment_id in comment_ids:
        status, data = api("GET", f"comment/{comment_id}", token=manage_sources_token)
        assert status == 200
...
def test_delete_source_cascade_groupsource(
    upload_data_token, manage_sources_token, public_group
):
    source_id = str(uuid.uuid4())
    ...
    status, data = api("GET", f"groups/{public_group.id}", token=manage_sources_token)
    assert len(data["data"]["group"]["sources"]) == 1

    status, data = api("DELETE", f"sources/{source_id}", token=manage_sources_token)
    assert status == 200
    status, data = api("GET", f"sources/{source_id}", token=manage_sources_token)
    assert status == 400
