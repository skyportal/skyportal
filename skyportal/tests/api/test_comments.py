...
def test_add_and_retrieve_comment_group_id(comment_token, public_source, public_group):
    ...
    status, data = api('GET', f'comment/object/{comment_id}', token=comment_token)
...
def test_add_and_retrieve_comment_group_access(
    ...
    status, data = api(
        'GET', f'comment/object/{comment_id}', token=comment_token_two_groups
    )
    assert status == 200
    assert data['data']['text'] == 'Comment text'
...
def test_delete_comment(comment_token, public_source):
    ...
    status, data = api('DELETE', f'comment/object/{comment_id}', token=comment_token)
    assert status == 200
...
def test_problematic_put_comment_attachment_1275(
    ...
    status2, data2 = api(
        'PUT',
        f'comment/object/{data["data"]["comment_id"]}',
        data={
    ...
    status3, data3 = api(
        'GET', f'comment/object/{data["data"]["comment_id"]}', token=super_admin_token
    )
    assert status3 == 200
