...
from skyportal.models import Thumbnail, DBSession, Source
...
def test_token_user_post_get_thumbnail(upload_data_token, public_group):
    ...
    orig_source_thumbnail_count = len(
        DBSession.query(Source).filter(Source.id == source_id).first().thumbnails
    )
    ...
    status, data = api(
        "POST",
        "thumbnail",
        data={"source_id": source_id, "data": data, "ttype": ttype},
        token=upload_data_token,
    )
    assert status == 200
    assert data["status"] == "success"
    ...
    assert (
        DBSession.query(Thumbnail)
        .filter(Thumbnail.id == thumbnail_id)
        .first()
        .source.id
    ) == source_id
    assert (
        len(DBSession.query(Source).filter(Source.id == source_id).first().thumbnails)
...
def test_cannot_post_thumbnail_invalid_size(upload_data_token, public_group):
    ...
    data = base64.b64encode(
        open(os.path.abspath("skyportal/tests/data/14gqr_new_50px.png"), "rb").read()
    )
    ttype = "ref"
    status, data = api(
        "POST",
...
def test_cannot_post_thumbnail_invalid_file_type(upload_data_token, public_group):
    ...
    data = base64.b64encode(
        open(os.path.abspath("skyportal/tests/data/phot.csv"), "rb").read()
    )
    ttype = "ref"
    status, data = api(
        "POST",
