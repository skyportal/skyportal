...
def test_filter_list(view_only_token, public_filter):
    ...
    assert all(k in data["data"][0] for k in ["query_string", "group_id"])
...
def test_token_user_retrieving_filter(view_only_token, public_filter):
    ...
    assert status == 200
    assert data["status"] == "success"
    assert all(k in data["data"] for k in ["query_string", "group_id"])
...
def test_token_user_post_new_filter(manage_groups_token, public_group):
    status, data = api(
        "POST",
        "filters",
        data={"query_string": str(uuid.uuid4()), "group_id": public_group.id,},
        token=manage_groups_token,
    )
    assert status == 200
