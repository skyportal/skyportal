...
def test_gcn_summary(
    ...
    # sources
    assert (
        "Found 1 sources in the event's localization, given the specified date range:"
        in data[6]
    )
    sources_table = data[7].split('\n')
    assert len(sources_table) == 6
    assert "id" in sources_table[1]
    assert "alias" in sources_table[1]
    assert "ra" in sources_table[1]
    assert "dec" in sources_table[1]
    assert "ra_err" in sources_table[1]
    assert "dec_err" in sources_table[1]
    assert "redshift" in sources_table[1]

    # source phot
    assert f"Photometry for source {obj_id}:"
    source_table = data[9].split('\n')
    assert len(source_table) == 6
    ...
    # galaxies
    assert "Found 82 galaxies in the event's localization:" in data[10]

    galaxy_table = data[11].split('\n')
    ...
    # obs
    assert "Observations:" in data[12]
    assert f"{name} - {instrument_name}" in data[13]
    ...
    assert obs_summary_text in data[13]

    obs_table = data[14].split('\n')
    assert len(obs_table) == 14
