...
def test_add_and_retrieve_comment_on_shift(public_group, super_admin_token, comment_token, super_admin_user):
  name = str(uuid.uuid4())
  ...
  status, data = api('GET', f'shift/{shift_id}/comments/{comment_id}', token=comment_token)

  assert status == 200
  assert data['data']['text'] == 'Comment on shift text'

def test_delete_comment_on_shift(comment_token, public_group, super_admin_token, super_admin_user):
  name = str(uuid.uuid4())
  start_date = date.today().strftime("%Y-%m-%dT%H:%M:%S")
  end_date = (date.today() + timedelta(days=1)).strftime("%Y-%m-%dT%H:%M:%S")
  request_data = {
  ...
  assert status == 403
  assert (
      "Could not find any accessible comments."
      in data["message"]
  )
