...
def test_add_stream_to_single_user_group_delete_stream(
    ...
    single_user_group = [group for group in data["data"]["all_groups"] if group["name"] == username][0]
    ...
    status, data = api(
        "GET", f"user/{new_user_id}", token=super_admin_token
    )
    assert status == 200
    assert data["data"]["id"] == new_user_id


def test_add_stream_to_group_delete_stream(super_admin_token, public_group, public_stream):
    status, data = api(
        "POST",
        f"groups/{public_group.id}/streams",
        data={
