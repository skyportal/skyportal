...
def test_token_user_post_get_photometry_data(upload_data_token, public_source):
    status, data = api(
        ...
        "photometry",
        data={
            "source_id": str(public_source.id),
...
def test_delete_photometry_data(upload_data_token, manage_sources_token, public_source):
    status, data = api(
        "POST",
        "photometry",
        data={
...
def test_delete_photometry_cascades_to_thumbnail(manage_sources_token, public_source):
    for phot in public_source.photometry:
        if len(phot.thumbnails) > 0:
            ...
            break
    assert (
        DBSession.query(Thumbnail).filter(Thumbnail.id == int(thumbnail_id)).count() > 0
    )
    ...
    assert (
        DBSession.query(Thumbnail).filter(Thumbnail.id == int(thumbnail_id)).count()
        == 0
    )
...
def test_token_user_post_photometry_thumbnail(upload_data_token, public_source):
    thumbnails = [
        {
            "data": base64.b64encode(
                open(
                    os.path.abspath(f"skyportal/tests/data/14gqr_{suffix}.png"), "rb"
                ).read()
            ),
            "ttype": suffix,
        }
    ...
    assert (
        len(
            DBSession.query(Photometry)
            .filter(Photometry.id == photometry_id)
            .first()
            .thumbnails
        )
        == 3
