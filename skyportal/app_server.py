...
skyportal_handlers = [
    ...
    (r"/api/allocation(/[0-9]+)?", AllocationHandler),
    (r"/api/analysis_service/([0-9]+)/default_analysis(/.*)?", DefaultAnalysisHandler),
    (r"/api/analysis_service(/[0-9]+)?", AnalysisServiceHandler),
    (
    ...
    (r"/api/assignment(/[0-9]+)?", AssignmentHandler),
    ...
    (r"/api/classification/votes(/[0-9]+)?", ClassificationVotesHandler),
    (r"/api/classification/sources(/[0-9]+)?", ObjClassificationQueryHandler),
    ...
    (
        r"/api/default_observation_plan(/[0-9]+)?",
        DefaultObservationPlanRequestHandler,
    ),
    (
        r"/api/default_survey_efficiency(/[0-9]+)?",
        DefaultSurveyEfficiencyRequestHandler,
    ),
    ...
    (r"/api/followup_request(/[0-9]+)?", FollowupRequestHandler),
    (r"/api/photometry_request(/[0-9]+)", PhotometryRequestHandler),
    ...
    (r"/api/earthquake(/[0-9]+)?", EarthquakeHandler),
    (r"/api/gcn_event(/.*)/alias", GcnEventAliasesHandler),
    (r"/api/gcn_event(/.*)/triggered(/[0-9]+)?", GcnEventTriggerHandler),
    (r"/api/gcn_event(/.*)/gracedb", GcnGraceDBHandler),
    (r"/api/gcn_event/(.*)/report(/[0-9]+)?", GcnReportHandler),
    (r"/api/gcn_event(/.*)/tach", GcnTachHandler),
    (r"/api/gcn_event/(.*)/summary(/[0-9]+)?", GcnSummaryHandler),
    (r"/api/gcn_event/(.*)/instrument(/[0-9]+)?", GcnEventInstrumentFieldHandler),
    ...
    (r"/api/localization(/.*)/notice(/[0-9]+)?", LocalizationNoticeHandler),
    ...
    (
        r"/api/groups(/[0-9]+)/usersFromGroups(/[0-9]+)?",
        GroupUsersFromOtherGroupsHandler,
    ),
    ...
    (r"/api/invitations(/[0-9]+)?", InvitationHandler),
    ...
    # End deprecated
    (r"/api/streams(/[0-9]+)/users(/[0-9]+)?", StreamUserHandler),
    ...
    (r"/api/internal/plot/airmass/assignment/([0-9]+)", PlotAssignmentAirmassHandler),
    (
