...
def photometry_plot(obj_id, user, width=600, height=300):
    ...
    ymax = (
        np.nanmax(
            (
                np.nanmax(data.loc[obsind, 'mag']) if sum(obsind) else np.nan,
                np.nanmax(data.loc[~obsind, 'lim_mag']) if sum(~obsind) else np.nan,
            )
        )
        + 0.1
    )
    ymin = (
        np.nanmin(
            (
                np.nanmin(data.loc[obsind, 'mag']) if sum(obsind) else np.nan,
                np.nanmin(data.loc[~obsind, 'lim_mag']) if sum(~obsind) else np.nan,
            )
        )
        - 0.1
    )
