...
def poll_alerts() -> None:
    ...
    maxtimeout = 5
    schema= os.path.abspath(os.path.join(os.path.dirname(__file__), '../../../data/fink_data/schemas/alert.avsc'))
        ...
        while True:
            ...
            if topic is not None:
                if alert is not None:
                    ...
                    post_fink_alerts.from_fink_to_skyportal(topic_to_classification(topic), topic_to_probability(topic), ztf_id, mjd, instrument, filter, mag, magerr, limiting_mag, magsys, ra, dec, token)
                    topic = None
                    alert = None
