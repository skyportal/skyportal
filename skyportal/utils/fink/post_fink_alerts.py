...
def post_classification(
    object_id, classification, probability, taxonomy_id, group_ids, token
):
    data = {
...
def from_fink_to_skyportal(
    ...
    if instrument_id is not None:
        ...
        if ztf_id not in candidate_ids:
            ...
            id_filter = 1
            post_candidate(ztf_id, ra, dec, [id_filter], passed_at, token=token)
        time.sleep(2)
        post_photometry(
            ztf_id,
            mjd,
        ...
        if classification_exists_for_objs(ztf_id, token=token):
            update_classification(
                ztf_id, classification, probability, taxonomy_id, [id_fink], token=token
            )
        else:
