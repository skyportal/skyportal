...
def check_payload(payload, station_name):
    ...
    if payload["end_date"] < str(datetime.now(UTC).replace(tzinfo=None)):
    ...
    def get(request, session, **kwargs):
        ...
        if not request.status.startswith("submitted: planified"):
            ...
            if not match_status or not match_status.groups():
                if observing_time > datetime.now(UTC).replace(tzinfo=None) + timedelta(
                    days=1
                ):
                ...
                elif observing_time >= datetime.now(UTC).replace(tzinfo=None):
        ...
        if not request.status.startswith("rejected") and observing_time < datetime.now(
            ...
            elif observing_time + timedelta(hours=3) < datetime.now(UTC).replace(
                tzinfo=None
            ):
                previous_status = (
                    "planified"
    ...
    def custom_json_schema(instrument, user, **kwargs):
        ...
        return {
            ...
            "properties": {
                ...
                "start_date": {
                    "type": "string",
                    "default": str(datetime.now(UTC).replace(tzinfo=None)).replace(
                        "T", ""
                    ),
                    "title": "Start Date (UT)",
                },
