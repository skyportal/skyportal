...
def load_demo_data(cfg, clear=False):
    ...
    with status(f"Creating dummy users"):
        ...
        super_admin_user = User.query.filter(
            User.username == "testuser@cesium-ml.org"
        ).first()
        ...
        view_only_user = User.query.filter(
            User.username == "viewonlyuser@cesium-ml.org"
        ).first()
    ...
    with status("Creating dummy instruments"):
        ...
        if not t1:
            ...
            i1 = Instrument(
                telescope=t1, name="P60 Camera", type="phot", band="optical"
            )
            ...
            i2 = Instrument(telescope=t2, name="ALFOSC", type="both", band="optical")
    ...
    with status("Creating dummy sources"):
        if not Source.query.get("14gqr"):
            ...
            for source_info in SOURCES:
                ...
                s.comments = [
                    Comment(text=comment, author=group_admin_user.username)
                ...
                s.photometry = [
                    Photometry(instrument=i1, **row) for j, row in phot_data.iterrows()
                ]
                ...
                for ttype in ["new", "ref", "sub"]:
                    ...
                    DBSession().add(t)
                    shutil.copy(
                        basedir / f"skyportal/tests/data/{fname}",
                        basedir / "static/thumbnails/",
                    )
