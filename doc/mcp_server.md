...
Replace `your-skyportal-host` with your instance URL (e.g., `fritz.science`, `localhost:5000`) and `your-api-token-here` with your API token.
...
claude mcp add --transport stdio skyportal_mcp \
  ...
  -- /path/to/skyportal/skyportal_env/bin/python -m skyportal.handlers.mcp
```

Replace:
- `/path/to/skyportal` with your local SkyPortal directory (in the Python path)
- `SKYPORTAL_URL` with the API endpoint (remote: `https://fritz.science`, local: `http://localhost:5000`)
...
The server lives in `skyportal/handlers/mcp/` and is built with [FastMCP](https://gofastmcp.com). It is part of the main web application: the `/mcp` route is a Tornado handler registered alongside the REST API handlers, so there is no separate service, port, or process to manage.

### How Requests Flow

MCP clients speak the Streamable HTTP transport: JSON-RPC messages POSTed to `/mcp`. The Tornado handler (`handler.py`):

1. Validates the `Authorization` header against SkyPortal's `Token` table (both `Bearer <token>` and `token <token>` schemes are accepted).
2. Stores the token in a request-scoped contextvar so tools can act as the caller.
3. Adapts the request to ASGI and dispatches it into FastMCP's stateless Streamable-HTTP app.

```
MCP Client                Tornado /mcp handler              FastMCP (ASGI)
    |                            |                               |
    |-- POST /mcp                |                               |
    |   Authorization: Bearer -->|-- Token table lookup          |
    |                            |-- set contextvar ------------>|
    |                            |                               |-- run tool
    |<- JSON-RPC response -------|<------------------------------|
```

The app is run in stateless mode: no server-side session state, so requests can land on any app worker behind the load balancer. Tools read data through the REST API using the caller's token, which means every request goes through the same permission checks as any other API client.

### stdio Mode (for local development)

The server can also run standalone as a subprocess, without starting SkyPortal at all — perfect for testing new tools against a remote instance:

```bash
MCP_TRANSPORT=stdio SKYPORTAL_URL=https://fritz.science \
  SKYPORTAL_TOKEN=your-api-token python -m skyportal.handlers.mcp
```

In stdio mode authentication uses the `SKYPORTAL_TOKEN` environment variable instead of per-request headers.

### Environment Variables

| Variable | Description | Default |
|----------|-------------|---------|
| `MCP_TRANSPORT` | Set to `"stdio"` for standalone subprocess mode | in-app HTTP |
| `SKYPORTAL_URL` | URL for SkyPortal's REST API | `http://localhost:{ports.app}` |
| `SKYPORTAL_TOKEN` | API token for stdio mode (ignored in-app) | None |

### Adding New Tools

Decorate any async function in a module under `tools/` with `@mcp.tool()`. Use `get_skyportal_token()` to retrieve the caller's API token (works in both modes):
...
Unit tests live in `skyportal/handlers/mcp/tests/` and run without a live SkyPortal instance: tools are exercised through FastMCP's in-memory client, SkyPortal API calls are intercepted with `httpx.MockTransport`, and the `/mcp` endpoint is tested with a stubbed token validator:

```bash
PYTHONPATH=. pytest skyportal/handlers/mcp/tests
```

### Trying It Against a Running Instance

Start SkyPortal normally (`make run`) and connect with Claude Code:

```bash
claude mcp add --transport http skyportal http://localhost:5000/mcp \
  --header "Authorization: Bearer your-api-token-here"
```
