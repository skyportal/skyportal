...
* TTT: The authentication takes the form `{"token": "token"}`. The token is provided by the following API request:

def get_user_token(user, password):
    url = "https://dto.ttt.lightbridges.es/api/"
    r = requests.post(url + "auth/get-token/", data={
        "username": user,
        "password": password
    }, verify=False)
