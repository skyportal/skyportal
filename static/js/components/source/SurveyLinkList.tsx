...
const SurveyLink = ({ name, url = null }: SurveyLinkProps) => {
...
interface SurveyLinkListProps {
  ra: number;
  dec: number;
  id: string;
}

const SurveyLinkList = ({ ra, dec, id }: SurveyLinkListProps) => {
  const { classes: styles } = useStyles();
  ...
  if (objPhotometry) {
    objPhotometry.forEach((phot: any) => {
      if (phot.mag) {
        isDetected = true;
