...
function GLS(t_data_uf: any, y_data_uf: any, kwa: any): any {
  let k;
  let i;
  const w: any[] = [];
  let wsum = 0;
  ...
  const t_data = goodi.map((ind: any) => t_data_uf[ind]);
  const y_data = goodi.map((ind: any) => y_data_uf[ind]);
  const e_y = goodi.map((ind: any) => kwa.e_y[ind]);

  const tmin = Math.min.apply(null, t_data);
  const t = add(t_data, -tmin) as any;

  // sorted time vector and the time differences between successive obs
  const tsort = [...t].sort((a, b) => a - b);
  const diffs = tsort
    ?.slice(1)
    ?.map((x: any, q: number) => x - tsort[q])
  ...
  const ymean = dot(w, y_data) as number;
  const y = add(y_data, -ymean) as any;
  const wy = dotMultiply(w, y) as any;
  const YY = dot(wy, y) as number; // Eq.(10), variance for the zero mean data
  ...
  const delta_t = Math.min(
    tbase / (nt - 1),
    quantileSeq(diffs, 0.75, false) as number,
    0.05 * ofac,
  );
...
const Periodogram = () => {
  ...
  function onZoom(minDate: number, maxDate: number) {
  ...
  function plotdata(times: any, mags: any, me: any) {
    const dat = transpose([times, mags])
      .map((x: any, i: number) => [x[0], [x[1], me[i]]])
      .sort((a: any, b: any) => b[0] - a[0]);
    const filteredy = mags?.filter((n: any) => n);
  ...
  function plotphased(times: any, mags: any, p: any, title: any) {
    // Create graph with native array as data source
    const pp = [
      ...times.map((x: any) => x % p),
      ...times.map((x: any) => (x % p) + p),
    ];
    const filteredy = mags?.filter((n: any) => n);
  ...
  function plotGLS(linear_freqs: any, linear_power: any, kwargs: any) {
    // Create graph with native array as data source

    // order by period ([day])
    const periods_all = linear_freqs
      .map((x: any) => Math.log10(1.0 / x))
      .reverse();
    const power_all = linear_power.reverse();

    // select only the range we want (< periodmax)
    const goodi = periods_all
      ?.map((e: any, i: number) => (e < kwargs.periodmax ? i : undefined))
      ?.filter((x: any) => x);
    const periods = goodi?.map((i: any) => periods_all[i]);
    const power = goodi?.map((i: any) => power_all[i]);
  ...
  useEffect(() => {
    ...
    if (photometry && establishedfilters && !run) {
      ...
      if (data.length > 0) {
        const times = data.map((x: any) => x.mjd);
        const mag = data.map((x: any) => x.mag);
        const magerr = data.map((x: any) => x.magerr);
        if (!plotted) {
    ...
    if (run) {
      ...
      const times = data.map((x: any) => x.mjd);
      const mag = data.map((x: any) => x.mag);
      const title = `P=${bestp * (periodmultiplier.toFixed(8) as any)} d (${
        params.instrument
      }: ${params.filter})`;
      if (times.length > 0) {
                        ...
                        <Grid size={12}>
                          <FormControl>
                            ...
                            <Controller
                              ...
                              {...({ className: classes.items } as any)}
                              render={() => (
                                <Select>
