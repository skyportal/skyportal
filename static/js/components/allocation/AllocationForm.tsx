...
interface ValidityRangeSelectProps {
  ranges?: any[];
...
const ValidityRangeSelect = ({
  ranges = [],
  onChange,
  errors,
  setErrors,
}: ValidityRangeSelectProps) => {
  ...
  const validateRanges = (rangesToProcess: any[]) => {
  ...
  const handleUpdate = (index: number, field: string, newValue: any) => {
  ...
  const [selectWidth, setSelectWidth] = useState<any>(null);
...
const AllocationForm = ({
  ...
  const [rangeErrors, setRangeErrors] = useState<Record<string, any>>({});
  ...
  useEffect(() => {
    if (instrumentList?.length > 0) {
      let options = instrumentList.map((instrument: any) => ({
        value: instrument.id,
  ...
  useEffect(() => {
    if (allocationId) {
      const allocation = allocationList.find(
        (alloc: any) => alloc.id === allocationId,
      );
      if (allocation) {
  ...
  const allocationFormSchema: any = {
    type: "object",
    properties: {
      group_id: {
        type: "integer",
        oneOf: (groups || []).map((group: any) => ({
          enum: [group.id],
          type: "integer",
          title: `${group.name}`,
        })),
      ...
      ...(availableUsers.length > 0 && {
        allocation_admin_ids: {
          ...
          items: {
            oneOf: availableUsers.map((user: any) => ({
              const: user.id,
          ...
          uniqueItems: true,
          default:
            allocationToEdit?.allocation_users.map((user: any) => user.id) ||
            [],
        },
      }),
      types: {
      ...
      instrument_id: {
        type: "integer",
        oneOf: instrumentOptions.map((instrument: any) => ({
  ...
  if (!groups || !instrumentOptions?.length) {
    return (
