...
function groupBy(arr: any[], key: string) {
  return arr.reduce((acc: Record<string, any[]>, x: any) => {
...
const calculateCentroid = (
  ...
  maxOffset = 0.5,
  sigmaClip = 4.0,
  snrThreshold: number | null = null,
): { refRA: number | null; refDec: number | null } => {
  ...
  const medianRA = d3.median(photometry.map((p) => p.ra)) as number;
  const medianDec = d3.median(photometry.map((p) => p.dec)) as number;
  ...
  if (points.length > 4 && sigmaClip > 0) {
    const std = d3.deviation(points.map((p) => p.offset_arcsec)) as number;
...
const prepareData = (
  ...
  refDec: number | null;
  points?: any[];
  oneSigmaCircle: number | null;
  stdCircle: number | null;
} => {
  if (!photometry || photometry.length === 0) {
  ...
  const std_dev_RA = d3.deviation(points.map((p) => p.deltaRA)) as number;
  const std_dev_Dec = d3.deviation(points.map((p) => p.deltaDec)) as number;
...
const CentroidPlot = ({
  ...
  const activeBrokers = (brokers ?? []).filter((b: any) => b.active);
  const coneSearchBrokerId = (
    activeBrokers.find((b: any) => b.default_crossmatch) ??
    activeBrokers.find((b: any) => b.capabilities?.cross_match_catalogs)
  )?.id;
