...
const SpectraPlot = ({
  ...
  const [vExpInput, setVExpInput] = useState<any>(0);
  const [redshiftInput, setRedshiftInput] = useState<any>(redshift || 0);
  const [smoothingInput, setSmoothingInput] = useState<any>(0);
  const [customWavelengthInput, setCustomWavelengthInput] = useState<any>(0);
  ...
  const findTypes = useCallback((spectraData: any[]) => {
    const spectrumTypes = new Set<any>();
  ...
  const prepareSpectra = useCallback(
    (spectraData: any[], spectrumTypes: any[]) => {
      const stats: any = {};
      ...
      const newSpectra = spectraData
        .map((spectrum) => {
          ...
          const fluxes_normed = spectrum.fluxes.map(
            (flux: number) => flux / normfac,
          );
  ...
  const createTraces = (
    ...
    if (spectraData !== null && specStats !== null) {
      ...
      const tracesOriginal = spectraFiltered.map((spectrum: any) => {
        const name = formatTraceName(spectrum);
        const existingTraceVisibility = existingPlotData
          ? existingTracesVisibilities[spectrum.id]
          : true;
      ...
      const redshift_value = parseFloat(redshiftInput) || 0;
  ...
  const createLineTraces = () => {
    ...
    const lineTraces = allLines
      .map((line: any) => {
        ...
        return line.x.map((x: number) => {
          const redshiftedX =
            line?.fixed === true
              ? x
              : x * (1 + (parseFloat(redshiftInput) || 0));
          const shiftedX =
            line?.fixed === true
              ? x
              : redshiftedX / (1 + (parseFloat(vExpInput) || 0) / C);
          return {
            type: "scatter",
            mode: "lines",
    ...
    let value = parseFloat(customWavelengthInput) || 0;
    const redshiftedX = value * (1 + (parseFloat(redshiftInput) || 0));
    const shiftedX = redshiftedX / (1 + (parseFloat(vExpInput) || 0) / C);
  ...
  useEffect(() => {
    if (plotData !== null && specStats !== null && types?.length > 0) {
      const redshift_val = parseFloat(redshiftInput) || 0;
      const vExp_val = parseFloat(vExpInput) || 0;
      ...
      const updatedPlotData = plotData.map((trace: any) => {
        ...
        const originalLine = allLines.find(
          (line: any) => trace.name === line.name,
        );
        if (originalLine) {
          const wavelength = parseFloat(trace.lineIdentifier.split("_").pop());
          const redshiftedX =
            (originalLine as any)?.fixed === true
              ? wavelength
  ...
  const toggleLine = useCallback((lineName: any) => {
      ...
      if (plotElement._fullLayout?.xaxis2) {
        try {
          ...
          if (
            ...
            (Plotly as any)
              .relayout(plotElement, { "xaxis2.range": newRange })
              .catch(() => {})
              .finally(() => {
  ...
  const handleLegendDoubleClick = (e: any) => {
    ...
    const nbAvailableTraces = e.data.filter(
      (trace: any) => trace.dataType === "Spectrum",
    ).length;
    ...
    const visibleTraces = e.data.filter(
      (trace: any) => trace.dataType === "Spectrum" && trace.visible === true,
    ).length;
    ...
    let spectrumVisibilityMap: any = {};

    // First pass: update Spectrum traces visibility
    let newTraces = e.data.map((trace: any, index: number) => {
      if (trace.dataType !== "Spectrum") {
        return trace;
      }
      let newVisible;
  ...
  const handleLegendSingleClick = (e: any) => {
    ...
    <div style={{ width: "100%", height: "100%" }} id="spectroscopy-plot">
      ...
      >
        <Plot
          ...
          onLegendDoubleClick={handleLegendDoubleClick as any}
          onLegendClick={handleLegendSingleClick as any}
