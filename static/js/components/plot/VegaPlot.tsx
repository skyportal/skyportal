...
const spec = (
  ...
  const hasDetections =
    !hasValuesArray ||
    values.some(
      (datum: any) => datum?.mag != null && Number.isFinite(Number(datum?.mjd)),
    );
...
const VegaPlot = React.memo((props: VegaPlotProps) => {
  const { dataUrl = null, colorScale, values = null, style = {} } = props;
