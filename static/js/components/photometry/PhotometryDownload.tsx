...
interface PhotometryDownloadProps {
  open: boolean;
  onClose: (...a: any[]) => void;
  data: any[];
  objId: string;
...
const PhotometryDownload = ({
  ...
  const handleValidationFilterChange = (key: string) => (event: any) => {
    setDownloadFormData((prev: any) => ({
      ...prev,
      validationFilter: {
  ...
  const filterDataByValidation = (tableData: any[], filter: any) => {
  ...
  const setColumnsForMode = (mode: string) => {
      ...
      .map((x) => {
        ...
        const streamsData =
          phot.streams?.length > 0
            ? phot.streams.map((s: any) => s.name).join(";")
            : "";
