...
interface EditSourceToolbarProps {
  setSourceRows: (...a: any[]) => void;
  setSourceRowModesModel: (...a: any[]) => void;
}

function EditSourceToolbar(props: EditSourceToolbarProps) {
  const { setSourceRows, setSourceRowModesModel } = props;

  const handleClick = () => {
    const id = randomId();
    setSourceRows((oldRows: any[]) => [
      ...oldRows,
      {
        id,
        obj_id: "",
    ...
    setSourceRowModesModel((oldModel: any) => ({
      ...oldModel,
...
export default function GcnReportEdit({ report }: GcnReportEditProps) {
  ...
  const [observationRows, setObservationRows] = React.useState<any[]>([]);
  ...
  React.useEffect(() => {
    ...
    const initialSourceRows = (data?.sources || []).map((source: any) => ({
      ...source,
      id: randomId(),
      obj_id: source?.id,
    }));
  ...
  const handleSave = () => {
    ...
    sourceRows.forEach((sourceRow) => {
      const source = data.sources.find((s: any) => s.id === sourceRow.obj_id);
      if (source) {
  ...
  const handleEditClick = (id: any, type: string) => () => {
    if (type === "source") {
      setSourceRowModesModel({
        ...sourceRowModesModel,
  ...
  const handleSaveClick = (id: any, type: string) => () => {
    if (type === "source") {
      setSourceRowModesModel({
        ...sourceRowModesModel,
  ...
  const handleCancelClick = (id: any, type: string) => () => {
    if (type === "source") {
      setSourceRowModesModel({
        ...sourceRowModesModel,
  ...
  const processSourceRowUpdate = (newRow: any) => {
  ...
  const handleSourceRowModesModelChange = (newRowModesModel: any) => {
  ...
  return (
    <div>
      ...
      >
        ...
        <div
          style={
            { display: "flex", direction: "row", alignItems: "center" } as any
          }
        >
      ...
      >
        <DataGrid
          ...
          slots={{
            toolbar: EditSourceToolbar as any,
          }}
          slotProps={{
            toolbar: { setSourceRows, setSourceRowModesModel } as any,
          }}
        />
      </Box>
      <div style={{ height: 20 }} />
