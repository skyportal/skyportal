...
export const addToFavorites = (source_id: string) =>
  API.POST("/api/listing", ADD_TO_FAVORITES, {
    list_name: "favorites",
    obj_id: source_id,
  });

export const removeFromFavorites = (source_id: string) =>
...
const reducer = (
  state: Record<string, any> = { favorites: [] },
  action: { type: string; data?: any },
): Record<string, any> => {
  switch (action.type) {
    case FETCH_FAVORITES_OK: {
      const favorites = action.data?.map((fav: any) => fav.obj_id);
      return {
        ...state,
        favorites,
      };
