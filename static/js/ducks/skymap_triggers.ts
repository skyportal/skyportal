...
export function requestAPISkymapTriggers(
  id: number | string,
  data: Record<string, any> = { triggersOnly: true },
) {
  return API.GET(
...
export function postAPISkymapTrigger(data: any) {
  return API.POST("/api/skymap_trigger", PUT_API_SKYMAP_TRIGGER, data);
}

export function deleteAPISkymapTrigger(
  id: number | string,
